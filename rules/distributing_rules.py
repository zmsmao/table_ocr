...
def rules(coord_img_path,table_engine,uuid,name):
    ...
    #开始匹配头部
    #查询侧面结果 判断是否有多画格子。
    # lattice_index,time_index = comm_rules.find_time_lattice(table_first[1][1],all_index)
    # if lattice_index==1:
    ...
    for i in range(len(ocr_result.index_)):
        ...
        if len(txt_filter_pj)>10:
            if txt_filter_pj[:10]=='安全措施或注意事项' or txt_filter_pj[:10].find('安全措施或注意事项')!=-1:
                ...
                continue
        #调度或设备运维单位负责的安全措施
        if txt_filter_pj[:16].find('调度')!=-1 or txt_filter_pj[:16].find('调或')!=-1:
            if  txt_filter_pj[:16].find('措施')!=-1 or txt_filter_pj[:16].find('措')!=-1:
                res=ocr_rules.get_txt_from_index(ocr_rules.find_cover_rect(s_index,all_index),all_txt)
